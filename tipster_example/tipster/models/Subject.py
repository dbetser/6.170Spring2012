...
class Subject(DatabaseObject):
    ...
    def get_reviews(self):
        ...
        return map(lambda row: Review(row, from_db=True), cur.fetchall())

    @staticmethod
    def get_by_name_and_category(subject_name, category_name):
    ...
    def get_recents(max):
        ...
        cur = g.db.execute('''select distinct subjects.* from subjects, reviews
                          where reviews.about = subjects.id order by reviews.created desc limit %s''' % max)
        return map(lambda row: Subject(row, from_db=True), cur.fetchall())
    ...
    @staticmethod
    def get_matches(subject_name, category_name):
        """
        Return list of subjects where name has subject_name as a substring
        ...
        return map(lambda row: Subject(row), cur.fetchall())
