...
class DatabaseObject(object):
    ...
    This code relies on a subtle property of Python dictionaries.
    Note that the SQL commands are created in two parts, one creating the body of the command,
    with a list of column names, and the other creating an array of values which are subsequently
    associated with those columns. Both parts are generated by iterating over the keys of a 
    dictionary. The order in which a dictionary yields its keys in the result of the built-in
    keys() method of the dictionary type is not specified in Python, but the order is guaranteed
    to be consistent across calls, so long as the dictionary is not modified in the interim.
    """
